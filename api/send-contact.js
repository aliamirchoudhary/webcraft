  ...
  try {
    await transporter.sendMail({
      ...
      html: `<!DOCTYPE html><html><head><style>${emailCSS}</style></head><body>
        <div class="wrapper">
          <div class="header"><h1>📬 New Contact Message</h1><p>Received via WebCraft contact form</p></div>
          <div class="body">
    ...
    await transporter.sendMail({
      ...
      html: `<!DOCTYPE html><html><head><style>${emailCSS}</style></head><body>
        <div class="wrapper">
          <div class="header"><h1>Thanks, ${escHtml(name)}! 👋</h1><p>Your message has been received.</p></div>
          <div class="body">
            <p>Hi ${escHtml(name)},</p>
            <p>Thanks for reaching out! I've received your message about <strong style="color:#99FFCC">"${escHtml(subject || 'General Inquiry')}"</strong> and will reply within <strong style="color:#99FFCC">24 hours</strong>.</p>
            <a class="btn" href="${process.env.FRONTEND_ORIGIN || 'https://your-site.vercel.app'}#services">Browse Services →</a>
          </div>
          <div class="footer"><p>WebCraft · <a href="mailto:${process.env.ADMIN_EMAIL}">${process.env.ADMIN_EMAIL}</a></p></div>
        </div></body></html>`,
    });
